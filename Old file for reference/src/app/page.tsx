...
export default function Home() {
  ...
  const handleInterestsChange = (value: string) => {
    setInterests(value);
    if (value.trim()) {
      localStorage.setItem("omegle-interests", value.trim());
    } else {
  ...
  const handleStartChat = (chatType: "text" | "video") => {
    // Save interests to localStorage for persistence
    if (interests.trim()) {
      localStorage.setItem("omegle-interests", interests.trim());
    } else {
      localStorage.removeItem("omegle-interests");
    }

    // Navigate to chat page with interests as URL parameter
    const interestsParam = interests.trim()
      ? `?interests=${encodeURIComponent(interests.trim())}`
      : "";
    router.push(`/${chatType}-chat${interestsParam}`);
  };
          ...
          <div className="grid md:grid-cols-2 gap-6">
            ...
            <div className="space-y-2">
              ...
              <button
                ...
                className="w-full bg-gray-200 hover:bg-gray-300 text-black p-2 rounded-lg text-left transition-colors text-sm cursor-pointer"
              >
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center space-x-3">
                <button
                  ...
                  className="flex-1 bg-blue-500 hover:bg-blue-600 text-white font-semibold py-3 px-4 rounded-lg transition-colors text-base chat-button cursor-pointer"
                >
                  Text
                </button>
                ...
                <button
                  onClick={() => handleStartChat("video")}
                  className="flex-1 bg-blue-600 hover:bg-blue-700 text-white font-semibold py-3 px-4 rounded-lg transition-colors text-base chat-button cursor-pointer"
                >
                  Video
                </button>
              </div>
